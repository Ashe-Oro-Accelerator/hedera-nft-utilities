...
export const uriDecoder = (nfts: NFTDetails | NFTDetails[], ipfsGateway?: string): DecodedMetadata[] => {
  ...
  const decodedMetadataArray: DecodedMetadata[] = nftsArray.map((nft: NFTDetails) => {
    ...
    if (!decodedNFTMetadata.startsWith('https://') && !decodedNFTMetadata.startsWith('http://') && !ipfsGateway) {
    ...
    } else if (!decodedNFTMetadata.startsWith('https://') && !decodedNFTMetadata.startsWith('http://') && ipfsGateway) {
